...
### RepeaterBook Proxy
RepeaterBook's API requires a specific `User-Agent` header to authorise requests. In development (`npm run dev`), Vite's built-in proxy handles this automatically. For production deployments, you will need a small server-side proxy (a single Fastify/Express endpoint) to forward requests with the correct header. This proxy can be deployed as a Docker container on any platform (Dokploy, Coolify, Railway, bare VPS, etc.) — no platform-specific primitives required.
...
> The application is a **100% client-side SPA** (Single Page Application). There is no backend server. All radio programming runs entirely in the browser via the Web Serial API. Local file open/save requires no network connection.
...
All core features (USB programming, file open/save, channel editing) work without any environment variables. The only optional variable is:

```
VITE_API_URL=https://your-proxy-host
```

Required only if you want RepeaterBook import to work in production. The browser cannot set the `User-Agent` header that RepeaterBook requires, so a small proxy sidecar is needed. See the Docker section in `FEATURE_PROPOSALS.md` for details.
...
The rest of the application — grid, settings panel, serial programmer — adapts automatically to the new driver's schema and capabilities.
...
- [ ] **Production deployment** — generic Docker-based deployment (frontend as static files + RepeaterBook proxy sidecar), compatible with any platform (Dokploy, Coolify, Railway, bare VPS, etc.)
...
We have identified **33 feature proposals** for future development, categorized by priority and type. For complete details, see **[FEATURE_PROPOSALS.md](FEATURE_PROPOSALS.md)**.
...
- Full read/write serial driver + `.ddmr` file format (DMR-specific columns: Timeslot, TalkGroup)
...
**Data Management**
- Multi-format export, Channel notes, Drag-and-drop
