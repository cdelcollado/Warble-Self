...
export async function codefilesRoutes(app: FastifyInstance): Promise<void> {
  // ── List ────────────────────────────────────────────────────────────────────
  app.get('/codefiles', async (req, reply) => {
  ...
  // ── Upload ──────────────────────────────────────────────────────────────────
  app.post('/codefiles', async (req, reply) => {
    ...
    const filePath = `${LOCAL_USER_ID}/${Date.now()}_${data.filename}`
    ...
    try {
      const [created] = await db.insert(codefiles).values({
        ...
        filePath,
        fileFormat: data.filename.match(/\.(img|csv|ddmr)$/i)?.[1]?.toLowerCase() ?? 'img',
        downloads: 0,
      }).returning()
  ...
  // ── Delete ──────────────────────────────────────────────────────────────────
  app.delete('/codefiles/:id', async (req, reply) => {
    const { id } = req.params as { id: string }
